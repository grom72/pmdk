...
main(int argc, char *argv[])
{
	...
	if (argc < 5)
		...
		"usage: %s <threads> <ops/t> <tx/t> <file> [enable stats]\
				[skip allocation increemnt steps]",
		argv[0]);

	PMEMobjpool *pop;
	unsigned enable_stats = 0, skip_allocation_inc = 0;
	...
	if (argc > 6)
		skip_allocation_inc = ATOU(argv[6]);
	...
	ret = pmemobj_ctl_get(pop, "stats.heap.curr_allocated", &allocPre);
	alloc = allocPre;
	if (enable_stats)
		alloc += Threads * 0x700;
	...
	ret = pmemobj_ctl_get(pop, "stats.heap.curr_allocated", &allocPost);
	if (alloc != allocPost) {
		alloc = allocPost - allocPre;
		if (alloc % 1792 != 0)
			UT_ASSERTeq(allocPre, allocPost);
	}
